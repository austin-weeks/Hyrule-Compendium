  ...
  useEffect(() => {
    async function loadEntries() {
      try {
        ...
        const fetches = [];
        fetches.push(
          fetchData("creatures").then(
            (entries) => entries !== null && all.push(...entries),
          ),
        );
        fetches.push(
          fetchData("monsters").then(
            (entries) => entries !== null && all.push(...entries),
          ),
        );
        fetches.push(
          fetchData("materials").then(
            (entries) => entries !== null && all.push(...entries),
          ),
        );
        fetches.push(
          fetchData("equipment").then(
            (entries) => entries !== null && all.push(...entries),
          ),
        );
        fetches.push(
          fetchData("treasure").then(
            (entries) => entries !== null && all.push(...entries),
          ),
        );
    ...
    <div className="size-full overflow-auto px-2 sm:px-4 flex flex-col">
      ...
      >
        {allEntries.map((entry) => (
          <EntryPreview searchString={search} entry={entry} key={entry.id} />
        ))}
      </div>
    </div>
  );
